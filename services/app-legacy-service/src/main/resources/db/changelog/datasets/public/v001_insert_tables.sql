...
--rollback DELETE FROM public.feature_flags WHERE key_flag IN ('ai_product_description', 's3_integration', 'email_sending', 'whatsapp_messaging', 'personalized_recommendations', 'proximity_sorting', 'discord_webhook');
...
--rollback DELETE FROM public.features_catalog WHERE feature_key IN ('comandalivre_self_order', 'comandalivre_access', 'prumodigital_access', 'admin_dashboard_access', 'whatsapp_integration', 'command_creation_access', 'product_modifiers_management', 'table_management_access', 'advanced_reporting_access', 'user_role_management');
...
--rollback DELETE FROM public.feature_groups WHERE group_key IN ('default_user_cl', 'default_user_pd', 'admin_system', 'developers', 'beta_testers', 'qa_team', 'premium_restaurant_cl', 'enterprise_project_pd');