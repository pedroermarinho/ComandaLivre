package io.github.pedroermarinho.comandalivreapi.company.core.domain.entities

import io.github.pedroermarinho.comandalivreapi.annotations.UnitTest
import io.github.pedroermarinho.comandalivreapi.company.core.domain.valueobject.EmployeeInviteStatus
import io.github.pedroermarinho.shared.valueobject.EntityAudit
import io.github.pedroermarinho.shared.valueobject.EntityId
import io.github.pedroermarinho.shared.valueobject.TypeKey
import io.github.pedroermarinho.shared.valueobject.TypeName
import org.junit.jupiter.api.Assertions.*
import org.junit.jupiter.api.DisplayName
import org.junit.jupiter.api.Test
import java.time.LocalDateTime
import java.util.UUID

@UnitTest
@DisplayName("Teste de unidade para EmployeeInviteStatusEntity")
class EmployeeInviteStatusEntityTest {
    @Test
    @DisplayName("Deve criar EmployeeInviteStatusEntity com todas as propriedades")
    fun shouldCreateEmployeeInviteStatusEntity() {
        val entityId = EntityId(internalId = 1, publicId = UUID.randomUUID())
        val audit =
            EntityAudit(
                createdAt = LocalDateTime.now(),
                updatedAt = LocalDateTime.now(),
                createdBy = "test",
                updatedBy = "test",
                deletedAt = null,
                version = 0,
            )
        val status =
            EmployeeInviteStatus(
                id = entityId,
                key = TypeKey("PENDING"),
                name = TypeName("Pending"),
                audit = audit,
            )

        assertEquals(entityId, status.id)
        assertEquals("PENDING", status.key.value)
        assertEquals("Pending", status.name.value)
        assertEquals(audit, status.audit)
    }
}
